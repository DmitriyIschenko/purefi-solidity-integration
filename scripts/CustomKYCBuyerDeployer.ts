...
async function deployBuyerWithCustomKYC(
  ...
  const ProxyFactory = await hardhat.getContractFactory(
  ...
  console.log(
    "KYCBuyerImplementation deployed at:",
    (await KYC1BuyerImplementation.deployed()).address,
  );

  const KYC1BuyerProxy = await ProxyFactory.connect(wallet).deploy(
    KYC1BuyerImplementation.address,
    ProxyAdmin.address,
    "0x",
  );
  console.log(
    "KYCBuyerProxy deployed at:",
    (await KYC1BuyerProxy.deployed()).address,
  );
  ...
  const setRuleIdTrans =
    await KYC1ProxyWithABI.connect(wallet).setRuleId(kycRule);
  const rec = await setRuleIdTrans.wait();
  console.log(
    `KYCBuyerProxy(${KYC1BuyerProxy.address}) setted ruleID as ${kycRule}`,
  );
...
async function main() {
  ...
  const rule81 = await deployBuyerWithCustomKYC(
    deployer,
    81n,
    tokenAddress,
    verifierAddress,
  );
  const rule82 = await deployBuyerWithCustomKYC(
    deployer,
    82n,
    tokenAddress,
    verifierAddress,
  ...
  const token = (
    await hardhat.getContractFactory("ERC20PresetMinterPauserUpgradeable")
  ).attach(tokenAddress);

  await (
    await token.connect(deployer).mint(rule81.address, 100000n * 10n ** 6n)
  ).wait(1);
  await (
    await token.connect(deployer).mint(rule82.address, 100000n * 10n ** 6n)
  ).wait(1);
