...
async function deployInfrastructure(wallet: ethers.Wallet) {
    ...
    const Implementation = await WhitelistProxyImplementation.connect(wallet).deploy();
    console.log("Implementation deployed at:", (await Implementation.deployed()).address);
    const Proxy = await WhitelistProxyFactory.connect(wallet).deploy(Implementation.address, ProxyAdmin.address, "0x");
...
async function intialize(proxyAddress:string) {
    const WhitelistProxyImplementation = (await hardhat.getContractFactory("IntegratedWhitelist")).attach(proxyAddress);
    const receipt = await WhitelistProxyImplementation.connect(deployer).initialize();
    await receipt.wait();
}
